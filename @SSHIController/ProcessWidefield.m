function ProcessWidefield(obj, YML)
...
if ~isempty(YML.directories.bf)
    ...
    out_bf = fullfile(YML.save_folder, YML.directories.bf, 'bf.tif');
...
if ~isempty(YML.directories.lasing)
    ...
    out_bf = fullfile(YML.save_folder, YML.directories.lasing, 'lasing.tif');
    obj.Processor.StitchWidefield(in_bf, out_bf, YML.stitch.bf);
end
end